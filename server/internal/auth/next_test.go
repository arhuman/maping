...
func TestSafeNext(t *testing.T) {
	cases := []struct{ in, want string }{
		...
		{"/", "/"},
		{"/dashboard", "/dashboard"},
		{"/billing/resume?plan=pro", "/billing/resume?plan=pro"},
...
func TestHandleStartStoresSanitizedNext(t *testing.T) {
	for _, c := range []struct{ query, wantNext string }{
		{"next=%2Fbilling%2Fresume%3Fplan%3Dpro", "/billing/resume?plan=pro"},
...
func TestCallbackReturningUserHonorsNext(t *testing.T) {
	rec := callbackWithNext(t, "/billing/resume?plan=pro")
	if rec.Code != http.StatusSeeOther {
		t.Fatalf("status = %d, want 303", rec.Code)
	}
	if loc := rec.Header().Get("Location"); loc != "/billing/resume?plan=pro" {
...
func TestDevLoginHonorsNext(t *testing.T) {
	...
	form := strings.NewReader("next=%2Fbilling%2Fresume%3Fplan%3Dteam")
	req := httptest.NewRequest(http.MethodPost, "/auth/dev/login", form)
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	rec := httptest.NewRecorder()
	a.handleDevLogin(rec, req)
	if loc := rec.Header().Get("Location"); loc != "/billing/resume?plan=team" {
...
func TestLoginPageCarriesNextIntoStartLinks(t *testing.T) {
	a := providerAuth(t, &fakeStore{}, "http://unused")
	rec := httptest.NewRecorder()
	a.handleLoginPage(rec, httptest.NewRequest(http.MethodGet, "/login?next=%2Fbilling%2Fresume%3Fplan%3Dpro", nil))
	...
	if !strings.Contains(body, "/auth/github/start?next=%2fbilling%2fresume%3fplan%3dpro") {
