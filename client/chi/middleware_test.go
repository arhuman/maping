...
func TestMiddlewareObservesMatchedRoute(t *testing.T) {
	...
	r := chi.NewRouter()
	r.Use(MiddlewareWithRecorder(rec))
	...
	assert.Equal(t, "/users/{id}", s.RouteTemplate, "must use the registered route pattern, never the raw path")
...
// TestMiddlewareObservesNestedRouterPattern is the "read after handler"
// correctness case: a subrouter contributes its mount prefix only as the handler
// chain unwinds, so the COMBINED pattern must be captured, not just the leaf.
func TestMiddlewareObservesNestedRouterPattern(t *testing.T) {
	fake := &fakeTransport{}
	rec := maping.NewRecorderForTest(fake)
	...
	req := httptest.NewRequest(http.MethodGet, "/api/things/7", nil)
	r.ServeHTTP(httptest.NewRecorder(), req)
	...
	sums := fake.summaries()
	require.Len(t, sums, 1)
	assert.Equal(t, "/api/things/{id}", sums[0].RouteTemplate, "nested router must yield the combined pattern")
}
...
func TestMiddlewareSkipsUnmatchedRoute(t *testing.T) {
	...
	r := chi.NewRouter()
	r.Use(MiddlewareWithRecorder(rec))
	// No route registered, so chi matches nothing → empty pattern → skip observe.
...
func TestMiddlewareReclassifiesCanceledRequest(t *testing.T) {
	...
	r := chi.NewRouter()
	r.Use(MiddlewareWithRecorder(rec))
	r.Get("/slow", func(_ http.ResponseWriter, _ *http.Request) {
